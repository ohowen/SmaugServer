...
/**
 * 
 * @author Hildenbrand, Nocaj ConvexClip used for computing intersection of two
 *         convex polygons in O(n+m)
 * 
 */
public class ConvexClip {
	...
	private void ConvexIntersection(cVertexList p, cVertexList q, int n, int m) {
		...
		do {
			...
			vQ = new Point2D(currq.v.x - currq.prev.v.x, currq.v.y
					- currq.prev.v.y);
			vP = new Point2D(currp.v.x - currp.prev.v.x, currp.v.y
					- currp.prev.v.y);
			...
			if (cross == 0 && pInQ < 0 && qInP < 0) {
				...
				/*
				 * Generic cases cross halfplane condition advance rule >0 qInP
				 * == 1 p >0 qInP == 0 q <0 pInQ == 1 q <0 pInQ == 0 p if p is
				 * advanced and inside flag = p then add currp to result
				 * (equivalent to q)
				 */
			} else if (cross >= 0) {
				if (qInP > 0) {
	...
	public InfoPoint intersect(Point2D a, Point2D b, Point2D c, Point2D d) {
		...
		t = (a.x * (d.y - c.y) - a.y * (d.x - c.x) + c.y * (d.x - c.x) - c.x
				* (d.y - c.y))
				/ denum;
		s = ((b.x - a.x) * a.y + c.x * (b.y - a.y) - a.x * (b.y - a.y) - c.y
				* (b.x - a.x))
				/ -denum;
		if (t >= 0 && t <= 1 && s >= 0 && s <= 1) {
			return new InfoPoint(new Point2D(a.x + t * (b.x - a.x), a.y + t
					* (b.y - a.y)), '1'); // Proper intersection
		}
